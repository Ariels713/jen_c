...
function Productions() {
  ...
  useEffect(() => {
    const aboutMe = async (event) => {
      try {
        const res = await fetch("/.netlify/functions/fitzpatrickTable");
        ...
        <Container>
          <Row>
            <Col className="ml-auto mr-auto" md="8">
              <h2 className="title">Emily Fitzpatrick Ceramics</h2>
              <h5>Product & Photography Stylist</h5>

              <h6 className="card-category ">
                Photographers: Rose Poage & Krysta Brayer
              </h6>
            </Col>
          </Row>
        </Container>
